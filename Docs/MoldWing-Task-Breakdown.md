...
M1 基础框架已完成，下一步开始实现 M2 框选功能：

### 🔥 优先：GPU 拾取系统（S2.1）
| ID | Task | 预计 |
|----|------|------|
| T2.1.1 | 创建 ID 渲染目标（R32_UINT格式） | 1h |
| T2.1.2 | 编写 ID 渲染着色器 | 1h |
| T2.1.3 | 实现 renderIDBuffer() | 1h |
| T2.1.4 | 实现 readFaceID(screenPos) | 1h |
| T2.1.5 | 实现 readFaceIDsInRect() | 1h |

### 选择系统核心（S2.2）
| ID | Task | 预计 |
|----|------|------|
| T2.2.1 | 创建 SelectionSystem 类（QObject） | 0.5h |
| T2.2.2 | 实现 selectedFaces 集合管理 | 0.5h |
| T2.2.3 | 定义 selectionChanged 信号 | 0.2h |
| T2.2.7 | 创建 SelectFacesCommand（QUndoCommand） | 1h |
