...
| 1.3 | 多 OBJ 批量导入 | File→Import Multiple 可选多文件 | ⏳ |
| 1.4 | 图层显示/隐藏 | 点击眼睛图标，模型消失/出现 | ⏳ |
| 1.5 | 图层选择高亮 | 点击图层项，对应模型边框高亮 | ⏳ |
| 1.6 | 修改状态标记 | 编辑纹理后，图层名旁显示 * 号 | ⏳ |
| 1.7 | 文件夹批量导入 | File→Import Folder | ⏳ |
...
M7.5 OBJ 带纹理导出已完成，下一步开始实现 M8 图层树与多模型管理：
...
|-----|------|-----------|
| **1.1** | 图层树 Dock 面板 | 左侧出现空的"图层"面板 |
| **1.2** | 单个 OBJ 显示在图层树 | 加载模型后树中显示文件名 |
| **1.3** | 多 OBJ 批量导入 | File→Import Multiple，可选多个文件 |
| **1.4** | 图层显示/隐藏 | 点击眼睛图标，模型消失/出现 |
| **1.5** | 图层选择高亮 | 点击图层项，对应模型边框高亮 |
| **1.6** | 修改状态标记 | 编辑纹理后，图层名旁显示 * 号 |
| **1.7** | 文件夹批量导入 | File→Import Folder |
...
| ID | Task | 预计 |
|----|------|------|
| T8.1.1 | 创建图层树 Dock 面板（QDockWidget） | 0.5h |
| T8.1.2 | 添加 QTreeWidget 作为图层树控件 | 0.5h |

**验收标准**：运行程序后，左侧 Tools 面板旁边出现一个名为"Layers"的空白 Dock 面板

---

**文档版本**: 5.0 (Qt 方案 - M8 图层树与多模型管理)
**最后更新**: 2025-12-30
