...
| 步骤 | 功能 | 可视化验收标准 | 状态 |
|-----|------|---------------|------|
| 3.1 | OSGB 导出菜单项 | File→Export OSGB | ⏳ |
| 3.2 | 单个 OBJ→OSGB | 生成 Data/tile_0/tile_0.osgb | ⏳ |
| 3.3 | 批量 OSGB 导出 | 多个 OBJ 各自生成瓦片目录 | ⏳ |

### S10.1 OSGB 格式研究 ⏳
| ID | Task | 预计 | 依赖 | 状态 |
|----|------|------|------|------|
| T10.1.1 | 研究 OSGB 文件格式规范 | 2h | M9 | ⏳ |
| T10.1.2 | 研究 osgDB 库集成方案 | 2h | T10.1.1 | ⏳ |
| T10.1.3 | vcpkg 添加 osg 依赖 | 1h | T10.1.2 | ⏳ |

### S10.2 OSGB 导出实现 ⏳
| ID | Task | 预计 | 依赖 | 状态 |
|----|------|------|------|------|
| T10.2.1 | 创建 OSGBExporter 类 | 1h | S10.1 | ⏳ |
| T10.2.2 | MeshData → osg::Geometry 转换 | 2h | T10.2.1 | ⏳ |
| T10.2.3 | 纹理导出到 OSGB 内嵌 | 1.5h | T10.2.2 | ⏳ |
| T10.2.4 | 写入 .osgb 文件 | 1h | T10.2.3 | ⏳ |

### S10.3 标准瓦片目录 ⏳
| ID | Task | 预计 | 依赖 | 状态 |
|----|------|------|------|------|
| T10.3.1 | 生成 Data/tile_xxx/ 目录结构 | 1h | S10.2 | ⏳ |
| T10.3.2 | 瓦片编号命名（tile_0, tile_1...） | 0.5h | T10.3.1 | ⏳ |
| T10.3.3 | 批量导出多个 OSGB | 1h | T10.3.2 | ⏳ |

### S10.4 UI 集成 ⏳
| ID | Task | 预计 | 依赖 | 状态 |
|----|------|------|------|------|
| T10.4.1 | File→Export OSGB 菜单项 | 0.5h | S10.3 | ⏳ |
| T10.4.2 | OSGB 导出对话框（复用 ExportDialog） | 0.5h | T10.4.1 | ⏳ |
| T10.4.3 | 进度显示和结果汇总 | 0.5h | T10.4.2 | ⏳ |
...
| M8 图层树与多模型管理 | 23h | 21h | 🔄 90% |
| M9 选择性导出 | 5h | 0h | ⏳ 0% |
| M10 OSGB标准瓦片导出 | 14h | 0h | ⏳ 0% |
| M11 橡皮擦 | 5h | 0h | ⏳ 0% |
| M12 颜色调整 | 12h | 0h | ⏳ 0% |
| M13 几何修复 | 16h | 0h | ⏳ 0% |
| **总计** | **~187h** | **~133h** | **~71%** |
...
| 单 OBJ 显示在图层树 | 加载模型后自动添加到图层树 |
| 多 OBJ 追加导入 | 多次打开文件，追加到 m_meshList |
| 异步加载 | QtConcurrent::run() 后台加载，UI 不卡顿 |
| 进度对话框动画 | QProgressDialog 动画正常显示 |
| 多模型渲染 | 每个模型独立 MeshRenderer 实例 |
| 图层显示/隐藏 | 勾选框控制模型可见性 |
| 合并包围盒 | 相机自动适配所有模型范围 |
| **CompositeId 工具类** | 高8位=meshId, 低24位=faceId 编码 |
| **多模型面拾取** | FacePicker 支持复合面ID，Alt+点击显示 Mesh:Face |
| **跨模型框选/刷选/套索/连通** | SelectionSystem 存储复合ID |
| **跨模型选择渲染** | SelectionRenderer 多模型高亮 |
| **多纹理编辑缓冲区** | MeshInstance.editBuffers 跨模型克隆 |
| **文件夹批量导入** | File→Import Folder (Ctrl+Shift+O) 递归导入 |
...
**验收标准**：导出对话框让用户勾选要导出的模型

---

**文档版本**: 5.4 (Qt 方案 - M8 方案B：B1-B5 + M8.1.1 完成)
**最后更新**: 2025-12-31
