...
M7 克隆图章工具已完成，下一步开始实现 M8 橡皮擦工具：

### 🔥 优先：橡皮擦核心（S8.1）
| ID | Task | 预计 |
|----|------|------|
| T8.1.1 | 创建 EraserTool 类（继承 TextureEditTool） | 0.5h |
| T8.1.2 | 保存原始纹理副本（用于恢复） | 0.5h |
| T8.1.3 | 实现橡皮擦绘制（恢复原始纹理） | 1.5h |
| T8.1.4 | 屏幕坐标→纹理坐标转换 | 0.5h |

### 橡皮擦优化（S8.2）
| ID | Task | 预计 |
|----|------|------|
| T8.2.1 | 实现笔刷大小调节 | 0.5h |
| T8.2.2 | 实现笔刷硬度（边缘羽化） | 0.5h |
| T8.2.3 | 属性面板：笔刷大小滑块 | 0.3h |
| T8.2.4 | 快捷键 E 激活橡皮擦 | 0.2h |

### 技术要点

**复用 M7 基础设施**：
- TextureEditBuffer：已有原始纹理备份（m_originalData）
- ScreenToTextureMapper：屏幕到纹理映射
- TextureEditCommand：撤销/重做支持
- MeshRenderer::updateTextureFromBuffer：GPU 纹理更新

**橡皮擦实现思路**：
1. 从 m_originalData 读取原始像素值
2. 写入到 m_data 当前编辑数据
3. 标记脏区域，同步到 GPU

---

**文档版本**: 4.0 (Qt 方案 - M7 克隆图章完成)
**最后更新**: 2025-12-30
